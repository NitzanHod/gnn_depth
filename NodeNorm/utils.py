...
def adjust_learning_rate(optimizer, epoch, lr_down_epoch_list):

    if epoch != 0 and epoch in lr_down_epoch_list:
        opt_name = list(dict(optimizer=optimizer).keys())[0]
        print('update learning rate of ' + opt_name)
...
def rewarp_graph(graph, config):
    ...
    if isinstance(graph, dgl.data.gnn_benchmark.CoauthorCSDataset) or \
       isinstance(graph, dgl.data.gnn_benchmark.CoraFull):
