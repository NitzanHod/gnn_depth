...
def get_activation(acti_type, acti_hyperparam):
    if acti_type == 'relu':
        activation = nn.ReLU(inplace=acti_hyperparam)
    elif acti_type == 'lkrelu':
        activation = nn.LeakyReLU(negative_slope=acti_hyperparam)
    elif acti_type == 'elu':
        activation = nn.ELU(inplace=acti_hyperparam)
    elif acti_type == 'no':
        activation = None
    else:
        raise NotImplementedError
...
class GNNBasicBlock(nn.Module):
    ...
    def forward(self, graph, x):
        ...
        if self.block_type == 'v':  # vanilla layers
