...
def cfg():
    ...
    data = {
        'dataset': dataset, 'implement':'pyg', 'add_slflp': True,
        'feature_prenorm': True, # whether to normalize raw features, applicable to both dgl and pyg
        'binarize': False, 'all_datasets': ['cora', 'citeseer', 'pubmed', 'coauthorcs'],
        'random_split': {'use': True, 'num_splits': 50}, 'label_per_class': 20,
        'full_sup': False,  # if true, use fully-supervised setting,
        'num_features': (features_classes.get(dataset))[0], 'num_classes': (features_classes.get(dataset))[1]
            }
    ...
    arch = {
        ...
        'layer_type': 'gcn',  # ['sage_res', 'gat_res', 'gcn', 'gat', 'sage']
        'block_type': 'v',
        'gat': {
            'num_heads': 8,
            'dropout': {
...
def get_structure(config):
    ...
    if config['arch']['dropout']['use'] and config['arch']['layer_type'] not in ['gat', 'gat_res']:
        ...
        dropout_idx = [x+1 for x in range(config['arch']['num_layers']-1)]  # x+2 to skip the GNNlayer & the activation
